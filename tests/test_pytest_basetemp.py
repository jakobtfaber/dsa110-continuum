...
checkout (``<rootdir>/.pytest_tmp``) via ``PYTEST_DEBUG_TEMPROOT``, so each
run gets its own numbered ``pytest-N`` directory with pytest's built-in
...
def test_default_basetemp_is_rooted_in_current_checkout(
    pytestconfig: pytest.Config, tmp_path_factory: pytest.TempPathFactory
) -> None:
    """Without explicit overrides, temp dirs land under <rootdir>/.pytest_tmp."""
    if pytestconfig.getoption("basetemp"):
        pytest.skip("explicit --basetemp override in effect")

    expected_root = Path(pytestconfig.rootpath, ".pytest_tmp").resolve()
