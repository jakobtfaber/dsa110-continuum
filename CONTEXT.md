...
- **Coadd** — combine regridded tile images into a single mosaic FITS. The canonical hourly-epoch coadd lives in `dsa110_continuum.mosaic` and preserves production batch semantics for WSClean beam-map blanking and strip grouping (`dsa110_continuum/mosaic/production.py::build_epoch_coadd`; `dsa110_continuum/mosaic/production.py::coadd_tiles`).
- **Strip grouping** (legacy day-batch) — `dsa110_continuum.mosaic.production.group_tiles_by_ra` partitions a day's tiles into contiguous RA strips with a 10° gap threshold. Used by the per-day batch path; the streaming path does not partition (it produces sliding-window mosaics directly) (`dsa110_continuum/mosaic/production.py::group_tiles_by_ra`).
